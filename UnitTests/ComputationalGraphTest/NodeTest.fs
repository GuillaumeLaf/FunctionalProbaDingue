module NodeTest
    ...
    let ``Create a new Parameter Vector of length 2`` () = 
        let actual = Vector.init Parameter (Group(0)) 2
        let expected = [| Input(Parameter(0,0)); Input(Parameter(0,1)) |] |> Vector
        Assert.Equal<Vector>(expected, actual)

    [<Fact>]
    let ``Add two Vectors`` () = 
        let v1 = Vector.init Parameter (Group(0)) 2
        let v2 = Vector.init Variable (Group(0)) 2
        let actual = v1 + v2
    ...
    let ``Create a new Parameter Matrix of dimensions (2,3)`` () = 
        ...
        let newVector grp = Vector.init Parameter (Group(grp)) 2
