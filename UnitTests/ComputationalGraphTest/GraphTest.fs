module GraphTest
    ...
    let ``run graph and treat indices as data`` () = 
        let g : Graph<int> = DataPoint(Parameter(0,0)) + DataPoint(Parameter(0,1)) + DataPoint(Parameter(0,2)) * DataPoint(Parameter(0,3))
    ...
    let ``Humongous graph to check StackOverflow`` () = 
        let g = Seq.init 10000 (fun _ -> DataPoint(Parameter(0,0))) |> sum
