module TimeSeriesTest
    ...
    let ``Get current element`` () = 
        let expected = Some [|Some 1f;Some 51f;Some 101f;Some 151f;Some 201f|]
        let actual = State.eval (TimeseriesState.currentElements) initialState
        Assert.Equal<float32 option [] option>(expected,actual)

    [<Fact>]
    let ``Get element at lag 1``() = 
        let expected = Some [|Some 0f;Some 50f;Some 100f;Some 150f;Some 200f|]
