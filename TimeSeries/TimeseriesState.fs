...
module TimeseriesState = 
    ...
    let inline size () = TS.size <!> timeseries()
    let inline length () = TS.length <!> timeseries()

    let inline lastIndex () = flip ( - ) 1 <!> length()

    // Set the current data
    let inline setData (newData:'T[,]) = State.modify (fun ((idx:int),ts) -> (idx, TS.setData newData ts))
    ...
    let inline currentElements () = TS.atTime< 'T > <!> currentTime() <*> timeseries()
